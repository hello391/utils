...
class Economy {
	...
	static async getUser(userID) {
		if (!config)
			throw new Error(
				'You have not setup the module, please put in a mongodb url'
			);
	...
	static async deposit(userID, amount) {
		...
		if (data.balance.wallet - amount < 0 && !config.allowBankruptcy)
			return false;
		if (data.balance.bank + amount > data.bankLimit && config.limits.enabled)
			return 'bank_limit';
		data.balance.wallet = data.balance.wallet - amount;
	...
	static async hasItem(userID, item) {
		...
		if (!data.find((i) => i.item.itemName.toLowerCase() === item.toLowerCase()))
			return false;
	...
	static async giveItem(userID, iName) {
		...
		const objIndex = await data.itemsOwned.findIndex(
			(obj) => obj.item.itemName === iName
		);
		let iowned;
		if (objIndex === -1) {
	...
	static async leaderboard(type) {
		...
		if (!type || type === 'both') {
			...
			d2.sort((a, b) => {
				return (
					parseFloat(b.balance.bank + b.balance.wallet) -
					parseFloat(a.balance.bank + a.balance.wallet)
				);
			});

			if (!d2[0] || d2[0].balance.bank + d2[0].balance.wallet === 0)
				return false;
	...
	static async rob(userID, robUserID, minEarn, maxEarn, failChance) {
		...
		if (!userID || !robUserID || !maxEarn || !failChance)
		...
		if (
			data2.balance.wallet - amount < amount - amount * 0.4 &&
			amount / 2 > data2.balance.wallet
		)
			return false; // Let them keep 4%
		data.balance.wallet = data.balance.wallet + amount;
	...
	static async sell(userID, iName) {
		...
		if (
			data.itemsOwned.findIndex((obj) => obj.item.itemName == item.name) === -1
		)
			return 'not_owned';
	...
	static async takeItem(userID, iName) {
		...
		if (!item) return false;
		let objIndex = data.itemsOwned.findIndex(
			(obj) => obj.item.itemName == item.itemName
		);
		if (objIndex === -1) return 'not_owned';

		if (data.itemsOwned[objIndex].amount === 1)
			data.itemsOwned = data.itemsOwned.filter((obj) => {
...
async function timeoutMsg(type, data) {
	if (type === 'daily')
		return (
			new Date(data.dailyTimeout).setDate(
				new Date(data.dailyTimeout).getDate() + 1
			) - Date.now()
		);
	if (type === 'weekly')
		return (
			new Date(data.weeklyTimeout).setDate(
				new Date(data.weeklyTimeout).getDate() + 7
			) - Date.now()
		);
	if (type === 'monthly')
		return (
			new Date(data.monthlyTimeout).setDate(
				new Date(data.monthlyTimeout).getDate() + 30
			) - Date.now()
		);
	return false;
}
